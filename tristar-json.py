...
while True:
    ...
    # Temperatures in C & F
        ...
        values["RTS_TempF"] = round(((rtsTemp - 32) * 5.0/9.0),1)  #(Fahrenheit - 32) * 5.0/9.0
        values["Heatsink_TempF"] = round(((hsTemp - 32) * 5.0/9.0),1)
        values["Battery_TempF"] = round(((batTemp - 32) * 5.0/9.0),1)
    # Solar Array
        values["Solar_Voltage"] = round((arrayV),2)
        values["Solar_Current"] = round((arrayI),2)
        values["Solar_Power"] = round((arrayW),2)
        values["Today's Production"] = round((whc_daily),2)
    # Logs & Status
        values["Faults"] = (faults[faultnum])
        values["Alarms"] = (alarm1, alarm2)
        values["Daily Flags"] = (flags_daily[flags_dailynum])
